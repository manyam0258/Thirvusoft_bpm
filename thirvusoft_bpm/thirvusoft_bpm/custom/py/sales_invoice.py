...
def create_payment_request(list_of_docs=None):
    ...
    for invoice in list_of_docs:
        ...
        # Get correct field
        installment_type = invoice_doc.get("custom_installment_type") or ""
        
        # Normalize (optional but safer)
        installment_type = installment_type.strip()
...
def fetch_guardian_email(doc):
    if doc.is_new():  # Only set values when the document is create
...
def set_advances(self, accounts):
    ...
    # If no advance exists for this customer → skip allocation
    if not advances:
        return
...
def get_advance_entries(self, accounts):
    ...
    # Loop through selected accounts, but DO NOT THROW if no advance
    for acc in accounts:
        party_accounts = [acc]
        ...
        #  Skip silently if no advance for this account
        if not advances:
            continue
